...
class NavigationPage(ctk.CTkFrame):
    def __init__(
        self, navigator: "NavigationManager", master: ctk.CTkFrame, title: str, **kwargs
    ):
...
class DefaultExceptionPage(NavigationPage):
    def __init__(
        self, navigator, master: ctk.CTkFrame, errorSupplier: ctk.StringVar, **kwargs
    ):
    ...
    def onShow(self):
        ...
        self.ui.get("error_message").getInstance().insert(
            "0.0", self.errorSupplier.get()
        )
...
class NavigationManager:
    ...
    def registerExceptionHandling(self, exceptionPage: "NavigationPage" = None):
        if self.exceptionPage:
            raise ValueError(
                f"An exception handler was already created! Type: {type(self.exceptionPage)}"
            )
    ...
    def getPage(self, page: Type[NavigationPage]) -> Optional[NavigationPage]:
        if not self.pageExists(page):
            if self.exceptionPage:
                self.exceptionMessageSupplier.set(
                    f"Page {page} is not registered with navigator {self.__class__}!"
                )
    ...
    def navigate(self, page: Type[NavigationPage]):
        ...
        if self.currentPage is not None:
            ...
            self._pageUnload(
                self.currentPage
            )  # give any page a chance to unload, regardless of ephemeral status
    ...
    def navigateEphemeral(self, page: "NavigationPage"):
        ...
        if not page.is_ephemeral:
            raise ValueError(
                "Ephemeral navigation can only be used with ephemeral pages."
            )
        if self.currentPage is not None:
            self.previousPage = self.currentPage
