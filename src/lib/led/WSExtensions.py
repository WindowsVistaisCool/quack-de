...
class SegmentedPixelStrip(ws.PixelStrip):
    ...
    def addSubStrip(self, start: int, end: int):
        for sub_strip in self.subStrips:
            ...
            assert not overlap, f"Sub-strip [{start}, {end}) overlaps with existing sub-strip [{sub_strip.start}, {sub_strip.end})"
    ...
    def getSubStrip(self, index: int):
        assert 0 <= index < len(self.subStrips), f"Sub-strip index {index} out of bounds"
...
class SubStrip:
    ...
    def setPixelColor(self, n, color):
        ...
        """
        tf = n + self.start
        assert self.start <= tf < self.end, f"Pixel index {n} out of bounds for sub-strip [{self.start}, {self.end})"
